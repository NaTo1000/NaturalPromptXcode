...
class XcodeProjectBuilder:
    ...
    def create_project(self, project: ProjectStructure, output_dir: str) -> str:
        ...
        if (
            hasattr(self.config, "batch_file_operations")
            and self.config.batch_file_operations
        ):
    ...
    def _batch_write_files(self, files: List, src_path: Path):
        ...
        for file in files:
            file_path = src_path / file.path
            with open(
                file_path, "w", buffering=8192
            ) as f:  # Larger buffer for performance
...
"""
    ...
    def build(self, project_path: str, configuration: str = "Debug") -> str:
        ...
        try:
            ...
            # Find built app
            app_path = (
                build_dir / "Build" / "Products" / configuration / f"{project_name}.app"
            )
            return str(app_path)
