...
class Config:
    ...
    def _load_from_file(self, config_file: str):
        ...
        if 'build' in data:
            self.ui_framework = data['build'].get('default_framework', self.ui_framework)
            self.language = data['build'].get('default_language', self.language)
            self.target_ios_version = data['build'].get('target_ios_version', self.target_ios_version)
        
        if 'output' in data:
            self.output_dir = data['output'].get('default_dir', self.output_dir)
            self.clean_before_build = data['output'].get('clean_before_build', self.clean_before_build)
